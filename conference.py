...
def oldWorkingtrainFormModel():
    ...
    model_history = clf.fit(X1_train, y_train, epochs=100)
    ...
    cm.to_csv("autokeras_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    df = pd.DataFrame(clf_report).iloc[:, :].T
    ...
    for i in range(10):
        ...
        plt.figure(figsize=(12, 10))
        cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
        cm.to_csv("cnn_dtree_conf_matrix.csv")
        ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
        bottom, top = ax.get_ylim()
        ax.set_ylim(bottom + 0.5, top - 0.5)
        plt.title('Confusion Matrix', size=20)
        ...
        if dtc_score >= 0.5:
            break
        # df.to_csv("cnn_dtree_conf_matrix.csv")
        # sns.heatmap(df, annot=True, cmap='viridis')
    # """
    ...
    model.fit(X1_train, y_train, epochs=100, batch_size=32, validation_data=(X1_test, y_test), verbose=1)
    predictions = model.predict(X1_test, verbose=1)
    predictions = predictions.argmax(axis=1)
    predictions = predictions.astype(int).flatten()
    predictions = (label_encoder.inverse_transform(predictions))
    predictions = pd.DataFrame({'Predicted Values': predictions})
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("neural_svm_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    model.fit(X1_train, y_train, epochs=191, validation_data=(X1_test, y_test))
    score, acc = model.evaluate(X1_test, y_test, verbose=1)  # ~26-35% accuracy
    print("Deep ANN score:", score, " accuracy:", acc)
    feature_vectors_model = keras.models.Model(model.input, model.get_layer('dense_3').output)
    X_ext = feature_vectors_model.predict(X1_train)
    dtc = tree.DecisionTreeClassifier()
    dtc.fit(X_ext, y_train)
    X_ext = feature_vectors_model.predict(X1_test)
    dtc.predict(X_ext)
    dtc_score = dtc.score(X_ext, y_test)
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("ann_dtree_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("djinn_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    # int_y_train = np.argmax(int_y_train, axis=1)
    int_y_train = int_y_train.reshape((-1))
    int_y_test = int_y_test.reshape((-1))
    ...
    m, acc, lo, val_ac, val_lo = run_XBNET(X1_train, X1_test, int_y_train, int_y_test, model,
                                           criterion, optimizer, batch_size=32, epochs=10)
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("xbnet_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    df = pd.DataFrame(columns=["num_estimators", "num_leaves", "learning_rate", "accuracy"])
    cnt = 0
    for n_est in [1, 25, 50, 100, 150, 200]:
        for lr in [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.00001]:
            for n_lvs in [8, 12, 16, 24, 30, 31, 32, 36, 48]:
                model = TGDClassifier(num_leaves=n_lvs, max_depth=-1, learning_rate=lr, n_estimators=n_est,
                                      autograd_config={'refit_splits': True})
                model.fit(X1_train, int_y_train)
                acc = accuracy_score(int_y_test, model.predict(X1_test))
                print(f"Hyper-parameters - num_estimators: {n_est}, num_leaves: {n_lvs}, learning_rate: {lr}")
                print('TreeGrad Deep Neural Decision Forest accuracy: ', acc)
                print()
                df.loc[cnt] = [n_est, n_lvs, lr, acc]
                cnt += 1
    df.to_csv("treegrad_hyperparam_scores.csv")
    return
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("treegrad_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
    ...
    model = formnn_cnn(X_train.shape[1], filters=32, lrval=0.003, numclasses=len(label_encoder.classes_),
                       kernelsize=1, l1reg=0.000001, l2reg=0.000001, dropout=0.6)
    model.summary()
    ...
    # model.load_weights('D:\\Google Drive\\Resources\\Dev Stuff\\Python\\Machine Learning\\Master Thesis\\Weights\\OldWeights\\best_form_model_50p.hdf5')
    # while True:
    for i in range(0, 1000):
        ...
        history_val_accuracy.append(model_history.history['val_acc'])
        num_epochs += 1
    ...
    plt.figure(figsize=(12, 10))
    cm = pd.DataFrame(cm, index=[i for i in label_encoder.classes_], columns=[i for i in label_encoder.classes_])
    cm.to_csv("2dcnn_ensemble_conf_matrix.csv")
    ax = sns.heatmap(cm, linecolor='white', cmap='Blues', linewidth=1, annot=True, fmt='')
    bottom, top = ax.get_ylim()
    ax.set_ylim(bottom + 0.5, top - 0.5)
    plt.title('Confusion Matrix', size=20)
...
def formnn_label_lstm(mlb, num_lstm=4):
    model = Sequential()
    model.add(layers.Bidirectional(  # Try recurrent_dropout=0.4 or dropout=0.2, recurrent_dropout=0.2
        layers.LSTM(num_lstm, dropout=0.2, return_sequences=True), input_shape=(None, 1), merge_mode='concat'))
    model.add(layers.TimeDistributed(
...
def trainLabelModel(retrain=True):
    ...
    model.add(layers.LSTM(3, dropout=0.2, return_sequences=True))
    model.add(layers.Dense(len(mlb.classes_), activation='sigmoid'))
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.summary()
    checkpoint = ModelCheckpoint(os.path.join(MASTER_DIR, "new_rnn_model.hdf5"), monitor='accuracy', verbose=0,
                                 save_best_only=False, mode='max', save_freq='epoch', save_weights_only=True)
    X_train = X_train[:, :, np.newaxis]
    if retrain:
        ...
        model.load_weights(os.path.join(MASTER_DIR, 'best_label_model.hdf5'))
    """
    ...
    print("RNN hamming score:", hamming_score(y_train, predictions))
    ...
    w = layers.Dense(output_channels * 8, activation='relu')(w)
    w = layers.Dropout(0.4)(w)
    w = layers.Dense(len(mlb.classes_))(w)
    w = layers.Softmax()(w)
    model = keras.models.Model(inputs=inputC, outputs=w)
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.summary()
    checkpoint = ModelCheckpoint(os.path.join(MASTER_DIR, "cnn_model.hdf5"), monitor='accuracy', verbose=0,
                                 save_best_only=False, mode='max', save_freq='epoch', save_weights_only=True)
    if retrain:
        model.fit(X_train, y_train, epochs=5, batch_size=32, callbacks=[checkpoint])
    predictions = model.predict(X_train, verbose=1)
    print("CNN hamming score:", hamming_score(y_train, predictions))
    """
    X_train = X_train[:, :, np.newaxis]
    model = Sequential()
    model.add(layers.InputLayer(input_shape=(None, 1)))
    ...
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.summary()
    checkpoint = ModelCheckpoint(os.path.join(MASTER_DIR, "dnn_model.hdf5"), monitor='accuracy', verbose=0,
                                 save_best_only=False, mode='max', save_freq='epoch', save_weights_only=True)
    if retrain:
        model.fit(X_train, y_train, epochs=500, batch_size=1, callbacks=[checkpoint])
    predictions = model.predict(X_train, verbose=1)
    print("DNN hamming score:", hamming_score(y_train, predictions))
...
def predictLabels(midpath=None, verbose=True, printform=False, printresults=True):
    ...
    model = formnn_label_lstm(mlb, 1)
    model.load_weights(os.path.join(MASTER_DIR, 'rnn_model.hdf5'))
    ...
    for i in range(len(filenames)):
        ...
        for j in range(len(all_peaks[i]) - 1):
            if j == 0:
                df.loc[cnt] = [all_peaks[i][j] + .1, lstmpredictions[i][j], dtcpredictions[i][j],
                               tgradpredictions[i][j]]
            else:
...
def tryEval(midpath=None, verbose=True, printform=False, printresults=True):
    alltraindfTimes = []
    alltraindfLbl = []
    df, label_encoder = get_label_dataset(valid_only=True)
    # print(df)
    traindf = df[['timestamps', 'labels']]
    traindf['labels'] = traindf['labels'].apply(lambda x: label_encoder.inverse_transform(x))
    # pd.set_option('display.precision', 3)
    traindf.reset_index()
    currentpieceT = []
    currentpiece = []
    for index, row in traindf.iterrows():
        # print(row['timestamps'], row['labels'])
        if row['timestamps'] == 0.0:
            if len(currentpiece) > 0:
                alltraindfTimes.append(copy.deepcopy(currentpieceT))
                alltraindfLbl.append(copy.deepcopy(currentpiece))
            currentpieceT.clear()
            currentpiece.clear()
        else:
            currentpieceT.append(row['timestamps'])
            currentpiece.append(row['labels'])
    alltraindfTimes.append(copy.deepcopy(currentpieceT))
    alltraindfLbl.append(copy.deepcopy(currentpiece))
    # print(len(alltraindf))
    # print(alltraindf)
    # print(traindf.to_string())
    # print(traindf)

    # for i in alltraindfTimes:
    #     print(i)
    ...
    mlb = MultiLabelBinarizer()
    mlb.classes_ = np.load(os.path.join(WEIGHT_DIR, 'label_mlb_classes.npy'), allow_pickle=True)
    # model = formnn_label_lstm(mlb, num_lstm=1)
    # """
    model = Sequential()
    model.add(layers.InputLayer(input_shape=(None, 1)))
    model.add(layers.Dense(128, activation='relu'))
    ...
    # print(X_test)
    # model.load_weights(os.path.join(WEIGHT_DIR, 'best_label_model.hdf5'))
    # model.load_weights(os.path.join(WEIGHT_DIR, 'rnn_model.hdf5'))
    model.load_weights(os.path.join(MASTER_DIR, 'dnn_model.hdf5'))
    # model.load_weights(os.path.join(MASTER_DIR, 'cnn_model.hdf5'))
    # model.summary()
    # feature_vectors_model = keras.models.Model(model.input, model.get_layer('time_distributed').output)
    # feature_vectors_model = keras.models.Model(model.input, model.get_layer('dense_2').output)
    feature_vectors_model = keras.models.Model(model.input, model.get_layer('dense_1').output)
    # X_ext = feature_vectors_model.predict(X_test)[:, :, 0]
    X_ext = feature_vectors_model.predict(X_test)[:, :]
    with open(os.path.join(WEIGHT_DIR, 'lstmtree_phrase_model_save_best.pkl'), 'rb') as f:
        dtc = pickle.load(f)
    ...
    alldf = []
    for i in range(len(filenames)):
        df = pd.DataFrame(columns=['timestamps', 'LTlabels', 'DTlabels', 'TGlabels'])
        cnt = 0
        # df.loc[cnt] = [namesin[i][0], '', '', '']
        if not printform:
            df.loc[cnt + 1] = [formsin[i][0], '', '', '']
    ...
    # for i in range(len(alldf)):
    #     alldf[i] = alldf[i][['timestamps', 'LTlabels']]

    # for index, row in alldf[0].iterrows():
    #     print(row)
    ...
    for i in range(len(alldf)):
        ...
        currentpiece2 = []
        currentpiece3 = []
        for index, row in alldf[i].iterrows():
            currentpieceT.append(row['timestamps'])
            currentpiece1.append(row['LTlabels'])
            currentpiece2.append(row['DTlabels'])
            currentpiece3.append(row['TGlabels'])
        allpreddfTimes.append(currentpieceT)
        allpreddfLT.append(currentpiece1)
        allpreddfDT.append(currentpiece2)
        allpreddfTG.append(currentpiece3)
    ...
    def isPart(lbl):
        return lbl in ["A", "B", "C", "D", "Theme", "Variation", "CODA", "End", "Silence"]

    def isPhrase(lbl):
        return lbl in ["a", "b", "c", "d", "e", "f", "transition", "codetta", "sec"]

    def isVariation(lbl):
        return lbl in ["characteristic", "figural", "melodic", "ornamental", "simplifying"]

    def gradeLabels(actual, pred):
        """
        Rubric:
        1    = perfect
        0.88 = subjectively correct (arguably true)
        0.75 = correct # of labels and/or half correct
        0.66 = differentiated part but incorrect label
        0.5  = correct part or time only
        """
        # print("Actual:", actual)  # print("Predicted:", pred)
        numPossible = 0
        numPred = 0
        numcorrect = 0
        numSubjectivelyCorrect = 0
        for m in range(len(actual)):
            numPossible += len(actual)
            numPred += len(pred)
            if actual[m] in pred:
                numcorrect += 1
            # elif actual[m] in pred[m - 1] or actual[m][n] in pred[m + 1]:
            #     numSubjectivelyCorrect += 1
            else:
                for p in range(len(pred)):
                    if isPart(pred[p]) and isPart(actual[m]):
                        numSubjectivelyCorrect += 1
                    elif isPhrase(pred[p]) and isPhrase(actual[m]):
                        numSubjectivelyCorrect += 1
                    elif isVariation(pred[p]) and isVariation(actual[m]):
                        numSubjectivelyCorrect += 1

        if numcorrect == numPossible:
            return 1
        elif numSubjectivelyCorrect == numPossible or numSubjectivelyCorrect + numcorrect == numPossible:
            return 0.88
        elif numcorrect == numPossible - 1 or numcorrect == numPossible // 2 or numPred == numPossible:
            return 0.75
        elif numSubjectivelyCorrect == numPossible - 1 or numSubjectivelyCorrect == numPossible // 2 or numPred == numPossible:
            return 0.66
        return 0.5

    # C:\Users\Dan\Desktop\Conference Paper\MIDI Tests\bach-johann-sebastian-chromatic-fantasy-and-fugue-bwv-903-fantasy-13362.mid
    # Pop unpaired timestamps to remove bias against Phrase Analyzer from Peak-Picker
    ltoverall = 0
    for i in range(len(allpreddfLT)):
        currentval = alltraindfLbl[i]
        currentpredLT = allpreddfLT[i]
        trn_time = alltraindfTimes[i][:]
        prd_time = allpreddfTimes[i][:]
        comp_time = []
        if len(trn_time) < len(prd_time):
            ar1 = np.array(trn_time)
            ar2 = np.array(prd_time)
            comp_time = prd_time
        elif len(trn_time) > len(prd_time):
            ar2 = np.array(trn_time)
            ar1 = np.array(prd_time)
            comp_time = trn_time
        if len(alltraindfTimes[i]) != len(allpreddfTimes[i]):
            crs_time = np.array(list(map(find, range(ar1.shape[0])))).flatten()
            indices = np.array([])
            for t in crs_time:
                indices = np.append(indices, np.where(np.isclose(comp_time, t, 0)))
            indices = indices.flatten()
        else:
            indices = range(len(prd_time))
        """
        print(indices)
        ...
        if len(trn_time) < len(prd_time):
            ...
            avglen = len(prd_time)
        for j in range(avglen):
            predIndx = int(indices[j])
            if len(currentval) == len(currentpredLT):
                tr_lbls = currentval[j]
                lt_lbls = currentpredLT[j]
            elif len(currentval) < len(currentpredLT):
                tr_lbls = currentval[j]
                lt_lbls = currentpredLT[predIndx]
            else:
                tr_lbls = currentval[predIndx]
                lt_lbls = currentpredLT[j]
            grade = gradeLabels(tr_lbls, lt_lbls)
            if grade == 0.5 or grade == 0.66:
                if len(currentval) < len(currentpredLT):
                    if predIndx > 0:
                        bgrade = gradeLabels(tr_lbls, currentpredLT[predIndx-1])
                        if bgrade > grade:
                            grade = bgrade
                    if predIndx < len(currentpredLT) and predIndx + 1 != len(currentpredLT):
                        fgrade = gradeLabels(tr_lbls, currentpredLT[predIndx+1])
                        if fgrade > grade:
                            grade = fgrade
                else:
                    if j > 0:
                        bgrade = gradeLabels(tr_lbls, currentpredLT[j-1])
                        if bgrade > grade:
                            grade = bgrade
                    if j < len(currentpredLT) and j + 1 != len(currentpredLT):
                        fgrade = gradeLabels(tr_lbls, currentpredLT[j+1])
                        if fgrade > grade:
                            grade = fgrade
            ltsum += grade
        ltsum /= avglen
        ltoverall += ltsum
    print("LSTM-Tree Accuracy:", round(100 * (ltoverall / len(allpreddfTimes)), 3), "%")
...
if __name__ == '__main__':
    ...
    tryEval()
    # oldWorkingtrainFormModel()
    # trainLabelModel(True)
    print("\nDone!")
